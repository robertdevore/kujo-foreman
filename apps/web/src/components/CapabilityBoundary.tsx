...
import { scopeLabel } from "../lib/presentation";
  ...
  return (
    <section className="panel" aria-labelledby="capability-title">
      ...
      <div className="table-scroll"><table className="capability-table"><caption className="sr-only">Capability calls made during this run</caption><thead><tr><th scope="col">Agent</th><th scope="col">Capability</th><th scope="col">Effect</th><th scope="col">Status</th></tr></thead><tbody>
        {capabilities.map((call) => <tr key={call.id}><td>{call.agent}</td><td><code>{call.capability}</code><small title={call.scope}>{scopeLabel(call.scope)}</small></td><td><span className={`effect effect--${call.effect}`}>{call.effect}</span></td><td><span className={`status-text status-text--${call.status}`}>{call.status}</span></td></tr>)}
      </tbody></table></div>
