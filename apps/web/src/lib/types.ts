...
export type StageStatus = "pending" | "active" | "passed" | "warning" | "failed";
...
export interface Stage extends Provenance {
  id: StageId;
  label: string;
  status: StageStatus;
  detail?: string;
...
export interface Verification extends Provenance {
  id: string;
  name: string;
  lane: "build" | "test" | "quality" | "security" | "behavior";
...
export interface CapabilityCall extends Provenance {
  id: string;
  agent: string;
  capability: string;
  effect: "read" | "execute" | "write" | "decision";
...
export interface Repair extends Provenance {
  id: string;
  status: "proposed" | "applying" | "verified" | "failed" | "rejected";
  path: string;
  reason: string;
  iteration: number;
}

export interface HumanDecision extends Provenance {
  id: string;
  question: string;
  reason: string;
  evidence: string[];
...
export interface ReleaseDecision extends Provenance {
  disposition: ReleaseDisposition;
  rationale: string;
  acceptanceCriteria: { satisfied: number; total: number };
